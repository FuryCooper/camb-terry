    ...
    if ( omnuh2 > omnuh2_sterile) then
        ...
        if (neutrino_hierarchy == neutrino_hierarchy_degenerate) then
            ...
            !P%Nu_mass_eigenstates=P%Nu_mass_eigenstates+1 !terry
            ...
            if (neutrino_hierarchy == neutrino_hierarchy_normal) then
                ...
                if (mnu > sqrt(delta_mnu31)+sqrt(delta_mnu31+delta_mnu21) + 1e-4_dl ) then
                    ...
                    P%Num_Nu_Massive = 2
		    print*, "incorrect mass limit? error in CAMB_SetNeutrinoHierarchy 2", mnu, sqrt(delta_mnu31)+sqrt(delta_mnu31+delta_mnu21) + 1e-4_dl
                end if
            else
            ...
            if (P%Num_Nu_Massive==3) then
                ...
                !P%Nu_mass_eigenstates = 2 !terry
                P%Nu_mass_degeneracies(1:3) = neff_massive_standard/3._dl !terry
                m3 = mnu - 2*m1
                P%Nu_mass_fractions(1:2) = m1/mnu*normal_frac !terry
                P%Nu_mass_fractions(3) = m3/mnu*normal_frac !terry
                P%Nu_mass_numbers(1:3) = 1 !terry
            else
		    !shouldn't happen with correct limit
		print*, "incorrect mass limit? error in CAMB_SetNeutrinoHierarchy"
		stop
