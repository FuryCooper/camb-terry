    ...
    type CAMBparams
        ...
        real(dl)  :: xi_nu_massless(3) !terry
  ...
  !L13=-0.0808025d0
  !L23=0.111343d0
  !CP%xi_nu(1) = L13*CP%xi_nu(3)
  !CP%xi_nu(2) = L23*CP%xi_nu(3)
  !do i = 1, 10
  !  CP%xi_nu(1) = CP%xi_nu(1) - (CP%xi_nu(1)*(CP%xi_nu(1)**2+pi**2)-L13*CP%xi_nu(3)*(CP%xi_nu(3)**2 + pi**2))/(pi**2+3.d0*CP%xi_nu(1)**2)
  !  CP%xi_nu(2) = CP%xi_nu(2) - (CP%xi_nu(2)*(CP%xi_nu(2)**2+pi**2)-L23*CP%xi_nu(3)*(CP%xi_nu(3)**2 + pi**2))/(pi**2+3.d0*CP%xi_nu(2)**2)
  !enddo
  !CP%xi_nu(1) = 0
  ...
  !print *, 'L1/L3', -(s12sq*r23+t13**2)/c12**2
    ...
    if (present(error)) then
        ...
        write(*,'("Om_g                 = ",es15.5)') grhog/grhom
        write(*,'("Neutrino degeneracy  = ",f9.6)') CP%xi_nu !Kenny: edited terry
        write(*,'("Om_m (1-Om_K-Om_L)   = ",f9.6)') 1-CP%omegak-CP%omegav
        write(*,'("100 theta (CosmoMC)  = ",f9.6)') 100*CosmomcTheta()
        write(*,*) "test 1st peak location",baumann_est(), 3.14/2/CosmomcTheta()
        if (CP%Num_Nu_Massive > 0) then
    ...
    function baumann_est()
    real(dl) zstar, astar, atol, rs, DA1,da2
    real(dl) CosmomcTheta
    real(dl) ombh2, omdmh2
    real(dl) rombint, tdec, baumann_est,r,cs
    external rombint
    ombh2 = CP%omegab*(CP%h0/100.0d0)**2
    omdmh2 = (CP%omegac+CP%omegan)*(CP%h0/100.0d0)**2
    zstar =  1048*(1+0.00124*ombh2**(-0.738))*(1+ &
    ...
    astar = 1/(1+zstar)
    R=3.0d4*astar*CP%omegab*(CP%h0/100.0d0)**2
    cs=1.0d0/sqrt(3*(1+R))
    atol = 1e-6
    tdec=DeltaTime(0.d0,astar)
    da1=AngularDiameterDistance(zstar)/astar
    da2=DeltaTime(astar,1.d0)
    baumann_est=3.14d0*da1/cs/tdec
    !!!!todo terry
    end function
...
500     if (L< lmax_extrap_highl) &
    ...
    integer, parameter  :: nrhopn=3000 !terry down from 80000; about the same dlnam as original
    !integer, parameter  :: nrhopn=80000 !terry
    real(dl), parameter :: am_min = 0.01_dl  !0.02_dl !Kenny: any problems? As Nu_background, Nu_rho and Nu_drho use approx. outside this range
    !smallest a*m_nu to integrate distribution function rather than using series
    real(dl), parameter :: am_max = 24000._dl           !Kenny: any problems? As Nu_background, Nu_rho and Nu_drho use approx. outside this range
...
!    subroutine Nu_init !Kenny: edited
    subroutine Nu_init(xi_camb) !Kenny: edited
      use constants !terry
    ...
    !print *, 'nu_init test xi',xi_camb, CP%Nu_mass_degeneracies(:CP%Nu_mass_eigenstates), CP%Nu_mass_eigenstates
    !    conv = k_B*(8*grhor/grhog/7)**0.25*CP%tcmb/eV * &
    !	    (CP%nu_mass_degeneracies(:)/CP%nu_mass_numbers(:))**0.25
    do i = 1, 4
...
!    if (allocated(r1)) return !Kenny: edited
    ...
    allocate(r1(nrhopn,4),p1(nrhopn,4),dr1(nrhopn,4),dp1(nrhopn,4),ddr1(nrhopn,4))                           !Kenny: edited terry
    ...
    if (nqmax==3) then
      ...
      do i = 1, 4
        nu_int_kernel(1:3,i) = (/0.0687359, 3.31435, 2.29911/)
      enddo
      ...
      do i = 1, 4
        nu_int_kernel(1:4,i) = (/0.0200251, 1.84539, 3.52736, 0.289427/)
      enddo
      ...
      do i = 1, 4
        nu_int_kernel(1:5,i) = (/0.0081201, 0.689407, 2.8063, 2.05156, 0.126817/)
      enddo
    else
      stop 'not supported yet, nqmax too big, AccuracyBoost>3'
      dq = (12 + nqmax/5)/real(nqmax)
      do i=1,nqmax
        q=(i-0.5d0)*dq
        nu_q(i) = q
        dlfdlq=-q/(1._dl+exp(-q))
            nu_int_kernel(i,1:4)=dq*q**3/(exp(q)+1._dl) * (-0.25_dl*dlfdlq) !now evolve 4F_l/dlfdlq(i) !terry
        end do
    end if
    if (nqmax>=3 .and. nqmax<=5) then !terry
      ...
      do i = 1, nqmax
        nu_int_kernel(i, :) = nu_w(i) * exp(nu_q(i)) * nu_q(i)**4 * exp(nu_q(i)-xi_camb(:)) / (1.d0 + exp(nu_q(i)-xi_camb(:)))**2 / 4 !terry
        nu_int_kernel(i, :) = nu_int_kernel(i,:) + nu_w(i) * exp(nu_q(i)) * nu_q(i)**4 * exp(nu_q(i)+xi_camb(:)) / (1.d0 + exp(nu_q(i)+xi_camb(:)))**2 / 4
        nu_int_kernel(i,:) = nu_int_kernel(i,:)/2
        !print *, 'test nu int kernel', nu_int_kernel(i)
        !print *, 'DEBUG: using nu_int_kernel with xi to 2nd order'
        !print *, 'DEBUG: using nu_int_kernel WITHOUT xi'
      enddo
    endif !terry
...
!    print*, "xi_camb=",  xi_camb    !!Kenny: problem!!! haven't been read after the first time!!!
!    print*, "r1(1000)=", r1(1000)   !!Kenny
!    print*, "p1(1000)=", p1(1000)   !!Kenny
...
enddo !terry
!print *, 'test nu_init', xi_fac(1,2) + const2*nu_masses(2)**2 * xi_fac(2,2), 1.d0 + const2*nu_masses(2)**2
    ...
    !cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!    subroutine nuRhoPres(am,rhonu,pnu) !Kenny: edited
    subroutine nuRhoPres(am,rhonu,pnu,xi_camb) !Kenny: edited
    ...
    ! Kenny: This is the point! rhonu and pnu computed should be a ratio relative to the energy density 
    ! and pressure of one flavor of the massless neutrino (relativistic fermion gas), and finally mutiple 
    ! by 7/8*(4/11)**4/3*result of photon gas (relativistic boson gas).
    ! Kenny: i.e. One flavor of neutrino vs. all photon 
    ! Kenny: i.e. 2 freedom (particle and anti-particle) vs. 2 freedom (spin)
    ! Kenny: So simply take average (sum of +Xi and -Xi divided by 2) is enough? 
    ! Kenny: Then the ratio of (+Xi + -Xi)/2 ->*2 freedom of photons->ratio of (+Xi + -Xi)*result of photons?

    !use constants !Kenny: edited
    ...
    real(dl), intent(in) :: xi_camb !Kenny: edited
...
!        aqdn=adq*q*q*q/(exp(q)+1._dl) !Kenny: edited
    ...
    rhonu_anti=(rhonu_anti+dum1_anti(nq+1)/adq)/const       !Kenny: edited
    pnu_anti=(pnu_anti+dum2_anti(nq+1)/adq)/const/3._dl     !Kenny: edited
    ...
    !rhonu_m_a_massless= 1._dl+30._dl*(xi_camb**2._dl)/7._dl/(const_pi**2._dl)+15._dl*(xi_camb**4._dl)/7._dl/(const_pi**4._dl) !Kenny: edited   
    !Kenny: (matter-massless int.+ anti_matter-massless int.)/2._dl/const
    
    !Print*, 'rhonu_m_a_massless in nuRhoPres', &
    !1._dl+30._dl*(xi_camb**2._dl)/7._dl/(const_pi**2._dl)+15._dl*(xi_camb**4._dl)/7._dl/(const_pi**4._dl) !Kenny: debug    

    end subroutine nuRhoPres

    !cccccccccccccccccccccccccccccccccccccccccc
    subroutine Nu_background(am,rhonu,pnu, nu_i) !terry
    use precision
    use ModelParams
    use constants
    real(dl), intent(in) :: am
    ...
    real(dl) d
    integer i
    real(dl) :: approx_p1, approx_p2, x2

    if (am <= am_minp) then
!        rhonu=1._dl + const2*am**2
        rhonu=xi_fac(1,nu_i) + const2*am**2 * xi_fac(2,nu_i)
!	d = (1._dl+30._dl*(CP%xi_nu(nu_i)**2._dl)/7._dl/(const_pi**2._dl)+15._dl*(CP%xi_nu(nu_i)**4._dl)/7._dl/(const_pi**4._dl)) !terry
!	rhonu = d + const2*am**2 * (1._dl + 3._dl / const_pi / const_pi * CP%xi_nu(nu_i)**2) !terry
        pnu=(2 * xi_fac(1,nu_i) - rhonu)/3._dl!terry
!        pnu=(2 - rhonu)/3._dl!terry
    ...
    !else if (am >= am_maxp) then
      ...
      rhonu = 3/(2*const)*(approx_p1*am + (approx_p2)/2/am)
      ...
      pnu = 900._dl/120._dl/const*(approx_p1-approx_p2/am**2)/am
      return
    else if (am >= am_maxp) then
    ...
    !  Cubic spline interpolation. terry
    ...
    end subroutine Nu_background !Kenny: one of the affected output of xi_nu

    !cccccccccccccccccccccccccccccccccccccccccc
    subroutine Nu_rho(am,rhonu, nu_i) !terry
    use precision
    use ModelParams
    use constants
    real(dl), intent(in) :: am
...
!	d = (1._dl+30._dl*(CP%xi_nu(nu_i)**2._dl)/7._dl/(const_pi**2._dl)+15._dl*(CP%xi_nu(nu_i)**4._dl)/7._dl/(const_pi**4._dl)) !terry
    ...
    -dr1(i+1,nu_i)+d*(dr1(i,nu_i)+dr1(i+1,nu_i)+2._dl*(r1(i,nu_i)-r1(i+1,nu_i)))))
    rhonu=exp(rhonu)
    end subroutine Nu_rho !Kenny: one of the affected output of xi_nu

    !ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc

    function Nu_drho(am,adotoa,rhonu,nu_i) result (rhonudot) !terry
    use precision
    use ModelParams
    use constants
    integer, intent(in) :: nu_i !terry
	...
	!d = (1._dl+30._dl*(CP%xi_nu(nu_i)**2._dl)/7._dl/(const_pi**2._dl)+15._dl*(CP%xi_nu(nu_i)**4._dl)/7._dl/(const_pi**4._dl)) !terry
!	d = (1._dl + 3._dl * (CP%xi_nu(nu_i)**2._dl) / const_pi / const_pi) !terry
	rhonudot = rhonudot * xi_fac(2,nu_i) !terry	
!	if (.not. printed) then
!		print *, 'am <= am_minp Nu_drho', am
!		printed = .true.
!	endif
    else if (am>am_maxp .and. CP%xi_nu(nu_i)/=0.d0) then
      x2=CP%xi_nu(nu_i)**2
      approx_p1 = zeta3 + 2.d0/3.d0*log(2.d0)*x2 + x2**2/72.d0 - x2**3/4320.d0 + x2**4/120960.d0
      approx_p2 = 15.d0*zeta5 + 6.d0*zeta3*x2 + 2.d0/3.d0*log(2.d0)*x2**2 + x2**3/180.d0 - x2**4/20160.d0
      rhonudot = 3/(2*const)*(approx_p1*am - (approx_p2)/2/am)*adotoa
    else if (am>am_maxp) then
        ...
        i=int(d)
        d=d-i
        !  Cubic spline interpolation for rhonudot. !terry
...
!        +2._dl*(dr1(i)-dr1(i+1)))))
    ...
    end function Nu_drho !Kenny: one of the affected output of xi_nu

    end module MassiveNu

    ! wrapper function to avoid cirular module references
!    subroutine init_massive_nu(has_massive_nu) !Kenny: edited
    subroutine init_massive_nu(has_massive_nu, xi_camb) !Kenny: edited
    use MassiveNu
    use ModelParams
    implicit none
...
!        print*, "4xi_camb=", xi_camb !Kenny !!!!!!!PROBLEM!!!!

!    if (has_massive_nu) then  !Kenny: edited
!print *, has_massive_nu
!print *, xi_camb
    if (has_massive_nu .or. any(xi_camb /= 0.d0)) then   !Kenny: edited terry
!        call Nu_Init !Kenny: edited
!        print*, "5xi_camb=", xi_camb !Kenny !!!!!!!PROBLEM!!!!
        call Nu_Init(xi_camb) !Kenny: edited
    else
        nu_masses = 0
    end if
    ...
    !outpower = exp(max(-30._dl,outpower)) !old camb
    ...
    if (CP%NonLinear/=NonLinear_none .and. CP%NonLinear/=NonLinear_Lens) then
        !call Transfer_GetMatterPowerData(MTrans, PK, in, itf, s1,s2) !old camb
