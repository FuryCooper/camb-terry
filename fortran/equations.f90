    ...
    call ThermalNuBack%rho_P(a*State%nu_masses(nu_i),rhonu,pnu, nu_i) !terry
    ...
    do nu_i = 1, CP%Nu_mass_eigenstates
        ...
        call ThermalNuBack%rho_P(a*State%nu_masses(nu_i),rhonu,pnu, nu_i)
        ...
        if (EV%MassiveNuApprox(nu_i)) then
            ...
            clxnu = clxnu/rhonu
            pinu=pinu/rhonu
            rhonudot = ThermalNuBack%drho(a*State%nu_masses(nu_i),adotoa, nu_i)
    ...
    do iq=1,EV%nq(nu_i)
        ...
        pinudot=pinudot+State%NuPerturbations%nu_int_kernel(iq, nu_i)*(ydot(ind)*v+y(ind)*vdot) !terry
        ind=ind+EV%lmaxnu_tau(nu_i)+1
    end do
    ind = EV%nu_pert_ix+2
    do iq=EV%nq(nu_i)+1,State%NuPerturbations%nqmax
        ...
        pinudot=pinudot+State%NuPerturbations%nu_int_kernel(iq, nu_i)*(psi2dot*v+psi2*vdot) !terry
    end do
    ...
    do iq=1, EV%nq(nu_i)
        ...
        aq=am/q
        v=1._dl/sqrt(1._dl+aq*aq)
        pinu=pinu+State%NuPerturbations%nu_int_kernel(iq, nu_i)*y(ind)*v !terry
    ...
    do iq=1, EV%nq(nu_i)
        ...
        aq=am/q
        v=1._dl/sqrt(1._dl+aq*aq)
        G11=G11+State%NuPerturbations%nu_int_kernel(iq, nu_i)*y(ind+1)*v**2 !terry
        if (EV%lmaxnu_tau(nu_i)>2) then
            G30=G30+State%NuPerturbations%nu_int_kernel(iq, nu_i)*y(ind+3)*v**2 !terry
        end if
        ind = ind+EV%lmaxnu_tau(nu_i)+1
    end do
    ...
    !terry: seems to use interpolation here, previous modification not needed?
